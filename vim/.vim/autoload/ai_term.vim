...
function! s:send_ctrl_v() abort
  let l:target_buf = bufnr('%')
  if l:target_buf <= 0 || !bufexists(l:target_buf)
    return ''
  endif

  if s:send_raw(l:target_buf, "\x16")
    return ''
  endif

  if exists('*term_sendkeys')
    call term_sendkeys(l:target_buf, "\x16")
  endif

  return ''
endfunction

function! s:paste_or_image() abort
  let l:target_buf = bufnr('%')
  if l:target_buf <= 0 || !bufexists(l:target_buf)
    return ''
  endif
  ...
  let l:text = getreg('+')
  if strlen(l:text) > 0
    if s:send_bracketed_paste(l:target_buf, l:text)
      return ''
    endif
    if exists('*term_sendkeys')
      call term_sendkeys(l:target_buf, l:text)
    endif
    return ''
  endif

  return s:send_ctrl_v()
endfunction
...
function! s:send_raw(buf, text) abort
  if !exists('*term_getjob') || !exists('*job_getchannel') || !exists('*ch_sendraw')
    return 0
  endif
...
function! s:define_buffer_maps() abort
  ...
  tnoremap <silent> <buffer> <C-v> <C-\><C-n>:call <SID>paste_or_image()<CR>i
  tnoremap <silent> <buffer> <D-v> <C-\><C-n>:call <SID>paste_or_image()<CR>i
endfunction
